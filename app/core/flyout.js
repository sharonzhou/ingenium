...
// ATTENTION: All *** indicates the changes necessary to construct a horizontal toolbox, by Sharon
    ...
    for (var i = 0, xml; xml = xmlList[i]; i++) {
      if (xml.tagName && xml.tagName.toUpperCase() == 'BLOCK') {
        ...
        if (block.getFieldValue("dropdown")) {
          ...
          // Do not allow moving until choose dropdown
          block.setMovable(false);
          block.movable_ = false;
          console.log('movable? shouldnt be', block, block.isMovable());
        }
        block.setDeletable(true);
        block.isDeletable = true;
        blocks.push(block);
        gaps.push(margin * 3);
      }
    }
...
Blockly.Flyout.prototype.reflow = function() {
  ...
  for (var x = 0, block; block = blocks[x]; x++) {
    ...
    // *** find tallest piece
    // *** flyoutHeight = Math.max(flyoutHeight, blockHW.height);
    // *** add widths of all blocks to determine flyout width, adding padding/margins between them later...
    // *** flyoutWidth += blockHW.width + margin * 2;
  }

  // *** flyoutHeight += margin + Blockly.BlockSvg.TAB_WIDTH + margin / 2 + Blockly.Scrollbar.scrollbarThickness;
...
Blockly.Flyout.prototype.blockMouseDown_ = function(block) {
  ...
  return function(e) {
    ...
    if (Blockly.isRightButton(e)) {
      ...
      // *** Instead, show gloss of word (tooltip)
      Blockly.Tooltip.show_();
    } else if (!block.isMovable()) {
      return;
    } else {
