...
Blockly.Block.prototype.fill = function(workspace, prototypeName) {
  ...
  this.nominativeOutputConnection = null;
  this.genitiveOutputConnection = null;
  this.dativeOutputConnection = null;
  this.accusativeOutputConnection = null;
  this.ablativeOutputConnection = null;
  this.vocativeOutputConnection = null;
...
Blockly.Block.prototype.unplug = function(healStack, bump) {
  ...
  if (this.outputConnection) {
    ...
    if (this.nominativeOutputConnection.targetConnection) {
      // Disconnect from any superior block.
      this.setParent(null);
    }
  }
  else if (this.genitiveOutputConnection) {
    if (this.genitiveOutputConnection.targetConnection) {
      // Disconnect from any superior block.
      this.setParent(null);
    }
  }
  else if (this.dativeOutputConnection) {
    if (this.dativeOutputConnection.targetConnection) {
      // Disconnect from any superior block.
      this.setParent(null);
    }
  }
  else if (this.accusativeOutputConnection) {
    if (this.accusativeOutputConnection.targetConnection) {
      // Disconnect from any superior block.
      this.setParent(null);
    }
  }
  else if (this.ablativeOutputConnection) {
    if (this.ablativeOutputConnection.targetConnection) {
      // Disconnect from any superior block.
      this.setParent(null);
    }
  }
  else if (this.vocativeOutputConnection) {
    if (this.vocativeOutputConnection.targetConnection) {
      // Disconnect from any superior block.
      this.setParent(null);
    }
  }
...
Blockly.Block.prototype.getConnections_ = function(all) {
  ...
  if (all || this.rendered) {
    ...
    if (this.nominativeOutputConnection) {
      myConnections.push(this.nominativeOutputConnection);
    }
    if (this.genitiveOutputConnection) {
      myConnections.push(this.genitiveOutputConnection);
    }
    if (this.dativeOutputConnection) {
      myConnections.push(this.dativeOutputConnection);
    }
    if (this.accusativeOutputConnection) {
      myConnections.push(this.accusativeOutputConnection);
    }
    if (this.ablativeOutputConnection) {
      myConnections.push(this.ablativeOutputConnection);
    }
    if (this.vocativeOutputConnection) {
      myConnections.push(this.vocativeOutputConnection);
    }
...
Blockly.Block.prototype.setParent = function(newParent) {
  if (this.parentBlock_) {
    ...
    //***
    if (this.nominativeOutputConnection && this.nominativeOutputConnection.targetConnection) {
      this.nominativeOutputConnection.disconnect();
    }
    if (this.genitiveOutputConnection && this.genitiveOutputConnection.targetConnection) {
      this.genitiveOutputConnection.disconnect();
    }
    if (this.dativeOutputConnection && this.dativeOutputConnection.targetConnection) {
      this.dativeOutputConnection.disconnect();
    }
    if (this.accusativeOutputConnection && this.accusativeOutputConnection.targetConnection) {
      this.accusativeOutputConnection.disconnect();
    }
    if (this.ablativeOutputConnection && this.ablativeOutputConnection.targetConnection) {
      this.ablativeOutputConnection.disconnect();
    }
    if (this.vocativeOutputConnection && this.vocativeOutputConnection.targetConnection) {
      this.vocativeOutputConnection.disconnect();
    }
...
//***
Blockly.Block.prototype.setNobOutput = function(inflectedCase, plurality, opt_check) {
  if (this.nominativeOutputConnection) {
    // Disconnect from parent and bump away, before removing children
    this.unplug(true, true);
    // Bump away children
    this.nominativeOutputConnection.dispose();
    this.nominativeOutputConnection = null;
  }
  if (this.genitiveOutputConnection) {
    // Disconnect from parent and bump away, before removing children
    this.unplug(true, true);
    // Bump away children
    this.genitiveOutputConnection.dispose();
    this.genitiveOutputConnection = null;
  }
  if (this.dativeOutputConnection) {
    // Disconnect from parent and bump away, before removing children
    this.unplug(true, true);
    // Bump away children
    this.dativeOutputConnection.dispose();
    this.dativeOutputConnection = null;
  }
  if (this.accusativeOutputConnection) {
    // Disconnect from parent and bump away, before removing children
    this.unplug(true, true);
    // Bump away children
    this.accusativeOutputConnection.dispose();
    this.accusativeOutputConnection = null;
  }
  if (this.ablativeOutputConnection) {
    // Disconnect from parent and bump away, before removing children
    this.unplug(true, true);
    // Bump away children
    this.ablativeOutputConnection.dispose();
    this.ablativeOutputConnection = null;
  }
  if (this.vocativeOutputConnection) {
    // Disconnect from parent and bump away, before removing children
    this.unplug(true, true);
    // Bump away children
    this.vocativeOutputConnection.dispose();
    this.vocativeOutputConnection = null;
  }
  ...
  if (inflectedCase && plurality) {
    if (opt_check === undefined) {
      opt_check = null;
    }
    if (inflectedCase == Blockly.inflectedCase.NOMINATIVE) {
      this.nominativeOutputConnection =
          new Blockly.Connection(this, Blockly.NOMINATIVE_OUTPUT_VALUE);
      this.nominativeOutputConnection.setCheck(opt_check);

    } else if (inflectedCase == Blockly.inflectedCase.GENITIVE) {
        this.genitiveOutputConnection =
          new Blockly.Connection(this, Blockly.GENITIVE_OUTPUT_VALUE);
      this.genitiveOutputConnection.setCheck(opt_check);

    } else if (inflectedCase == Blockly.inflectedCase.DATIVE) {
        this.dativeOutputConnection =
          new Blockly.Connection(this, Blockly.DATIVE_OUTPUT_VALUE);
      this.dativeOutputConnection.setCheck(opt_check);

    } else if (inflectedCase == Blockly.inflectedCase.ACCUSATIVE) {
        this.accusativeOutputConnection =
          new Blockly.Connection(this, Blockly.ACCUSATIVE_OUTPUT_VALUE);
      this.accusativeOutputConnection.setCheck(opt_check);

    } else if (inflectedCase == Blockly.inflectedCase.ABLATIVE) {
        this.ablativeOutputConnection =
          new Blockly.Connection(this, Blockly.ABLATIVE_OUTPUT_VALUE);
      this.ablativeOutputConnection.setCheck(opt_check);

    } else if (inflectedCase == Blockly.inflectedCase.VOCATIVE) {
        this.vocativeOutputConnection =
          new Blockly.Connection(this, Blockly.VOCATIVE_OUTPUT_VALUE);
      this.vocativeOutputConnection.setCheck(opt_check);
...
/**
 * Shortcut for appending a nominative value input row. ***
 * @param {string} name Language-neutral identifier which may used to find this
 *     input again.  Should be unique to this block.
 * @return {!Blockly.Input} The input object created.
 */
Blockly.Block.prototype.appendNominativeValueInput = function(name) {
  return this.appendInput_(Blockly.NOMINATIVE_INPUT_VALUE, name);
};

/**
 * Shortcut for appending a genitive value input row. ***
 * @param {string} name Language-neutral identifier which may used to find this
 *     input again.  Should be unique to this block.
 * @return {!Blockly.Input} The input object created.
 */
Blockly.Block.prototype.appendGenitiveValueInput = function(name) {
  return this.appendInput_(Blockly.GENITIVE_INPUT_VALUE, name);
};

/**
 * Shortcut for appending a dative value input row. ***
 * @param {string} name Language-neutral identifier which may used to find this
 *     input again.  Should be unique to this block.
 * @return {!Blockly.Input} The input object created.
 */
Blockly.Block.prototype.appendDativeValueInput = function(name) {
  return this.appendInput_(Blockly.DATIVE_INPUT_VALUE, name);
};

/**
 * Shortcut for appending a accusative value input row. ***
 * @param {string} name Language-neutral identifier which may used to find this
 *     input again.  Should be unique to this block.
 * @return {!Blockly.Input} The input object created.
 */
Blockly.Block.prototype.appendAccusativeValueInput = function(name) {
  return this.appendInput_(Blockly.ACCUSATIVE_INPUT_VALUE, name);
};

/**
 * Shortcut for appending a ablative value input row. ***
 * @param {string} name Language-neutral identifier which may used to find this
 *     input again.  Should be unique to this block.
 * @return {!Blockly.Input} The input object created.
 */
Blockly.Block.prototype.appendAblativeValueInput = function(name) {
  return this.appendInput_(Blockly.ABLATIVE_INPUT_VALUE, name);
};
 ...
 * @return {!Blockly.Input} The input object created.
 */
Blockly.Block.prototype.appendVocativeValueInput = function(name) {
  return this.appendInput_(Blockly.VOCATIVE_OUTPUT_VALUE, name);
};
...
Blockly.Block.prototype.appendInput_ = function(type, name) {
  ...
  // if (type == Blockly.INPUT_VALUE || type == Blockly.NEXT_STATEMENT) {
    // ***
  if (type == Blockly.INPUT_VALUE || type == Blockly.NEXT_STATEMENT
   || type == Blockly.NOMINATIVE_INPUT_VALUE || type == Blockly.GENTIIVE_INPUT_VALUE || 
   type == Blockly.DATIVE_INPUT_VALUE || type == Blockly.ACCUSATIVE_INPUT_VALUE || 
   type == Blockly.ABLATIVE_INPUT_VALUE || type == Blockly.VOCATIVE_INPUT_VALUE) {
