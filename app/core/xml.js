...
Blockly.Xml.blockToDom_ = function(block) {
  ...
  for (var i = 0, input; input = block.inputList[i]; i++) {
    ...
    if (input.type == Blockly.DUMMY_INPUT) {
      ...
      if (input.type == Blockly.INPUT_VALUE || input.type == Blockly.NOMINATIVE_INPUT_VALUE || input.type == Blockly.GENITIVE_INPUT_VALUE
        || input.type == Blockly.DATIVE_INPUT_VALUE || input.type == Blockly.ACCUSATIVE_INPUT_VALUE || 
        input.type == Blockly.ABLATIVE_INPUT_VALUE || input.type == Blockly.VOCATIVE_INPUT_VALUE) {
  ...
  for (var x = 0, xmlChild; xmlChild = xml.childNodes[x]; x++) {
    if (xmlChild.nodeName.toLowerCase() == 'block') {
      ...
      if (!isNaN(blockX) && !isNaN(blockY)) {
        console.log('not bumping' + blockX + blockY);
